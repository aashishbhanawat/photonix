...
def get_secret_key():
    ...
    if 'DJANGO_SECRET_KEY' in os.environ:
        ...
        if redis.redis_connection.exists('django_secret_key'):
            secret_key = redis.redis_connection.get(
                'django_secret_key').decode('utf-8')
        else:
            # Make sure only first worker generates the key and others get from Redis
            with Lock(redis.redis_connection, 'django_secret_key_generation_lock'):
                if redis.redis_connection.exists('django_secret_key'):
                    secret_key = redis.redis_connection.get(
                        'django_secret_key').decode('utf-8')
                else:
                    secret_key = utils.get_random_secret_key()
                    redis.redis_connection.set(
                        'django_secret_key', secret_key.encode('utf-8'))
