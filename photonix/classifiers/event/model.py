...
class EventModel:
    ...
    def predict(self, image_file):
        ...
        date_taken = None
        possible_date_keys = ['Date/Time Original', 'Date Time Original', 'Date/Time',
                              'Date Time', 'GPS Date/Time', 'Modify Date', 'File Modification Date/Time']
            ...
            if date_taken:
                ...
                if events.get(date_taken.date()):
                    if events.get(date_taken.date()).startswith("New Year"):
                        start_of_day = datetime.datetime.combine(datetime.date(
                            date_taken.year, 12, 31), datetime.datetime.min.time())
